...
function filterProductsByPriceRange(products, min, max) {
  ...
  if (min < 0 || max < 0) {
    throw new Error("Both `min` and `max` should be greater than or equal to 0.");
  }

  for (let product of products) {
    if (!product.priceInCents) {
      throw new Error("Some products in the `products` array do not have a `priceInCents` key.");
    }
  }
